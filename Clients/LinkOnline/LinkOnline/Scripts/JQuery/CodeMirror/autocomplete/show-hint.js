(function () {
    ...
    Completion.prototype = {
        ...
        pick: function (data, i) {
            var text = editor.getLine(data.to.line).trim();

            var parts = text.split('.', 100);

            text = parts[parts.length - 1];

            var from = {line:data.to.line, ch:(data.to.ch - text.length)};

            var completion = data.list[i];
            if (completion.hint) completion.hint(this.cm, data, completion);
                //else this.cm.replaceRange(getText(completion), data.from, data.to);
                //else this.cm.replaceRange(getText(completion), from, data.to);
            else
                this.cm.replaceRange(getText(completion), from, data.to);
    ...
    function Widget(completion, data) {
        ...
        editor.getWrapperElement().onkeypress = function (e) {
            var code = e.keyCode;

            if (code == 0)
                code = e.charCode;

            if (code != 46 && code != 59 && code != 32)
                return;
